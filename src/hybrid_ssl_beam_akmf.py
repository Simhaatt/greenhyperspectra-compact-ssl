...
RESULTS_DIR = ROOT / "results" / "hybrid_ssl_beam_akmf"
...
class KANLayer(nn.Module):
    """KAN-style radial spline layer used as a nonlinear projection block."""
...
class AKMFFusionRegressor(nn.Module):
    """Attentive KAN-MLP fusion head over BeamSearch bands plus SSL embeddings."""

    def __init__(self, in_features: int, config: AKMFConfig) -> None:
        super().__init__()
        self.config = config
        ...
        self.kan = KANLayer(in_features, config.kan_hidden, config.grid_size)
...
def fit_predict(
    ...
    model = AKMFFusionRegressor(x_train.shape[1], config).to(device)
...
def run_trait(
    ...
    print(f"\nTrait={target} | train={len(y_train)} test={len(y_test)} akmf_features={x_train.shape[1]} device={device}")
    for seed in seeds:
        pred, timing = fit_predict(x_train, y_train, x_test, seed, config, device)
        row = {
            "target": target,
            "selector": "BeamSearch",
            "model": "AKMFNet_Attentive_KAN_MLP_Fusion",
            "seed": seed,
            "budget": budget,
            "beam_bands": budget,
...
def save_figures(summary: pd.DataFrame, run_name: str) -> None:
    ...
    for metric in ["r2", "rmse"]:
        ...
        plt.title(f"AKMF-Net attentive KAN-MLP fusion: {metric.upper()}")
        plt.grid(True, alpha=0.25)
        plt.legend()
        plt.tight_layout()
        plt.savefig(FIG_DIR / f"hybrid_ssl_beam_akmf_{run_name}_{metric}.png", dpi=240)
        plt.close()


def main() -> None:
    parser = argparse.ArgumentParser(description="AKMF-Net: attention + KAN + MLP hybrid SSL/BeamSearch fusion.")
    ...
    results_path = RESULTS_DIR / f"hybrid_ssl_beam_akmf_{args.run_name}_results.csv"
    summary_path = RESULTS_DIR / f"hybrid_ssl_beam_akmf_{args.run_name}_summary.csv"
    results.to_csv(results_path, index=False)
    summary.to_csv(summary_path, index=False)
    summary.to_csv(SUMMARY_DIR / f"hybrid_ssl_beam_akmf_{args.run_name}_summary.csv", index=False)
    save_figures(summary, args.run_name)

    print("\nSaved:")
    print(results_path)
    print(summary_path)
    print(SUMMARY_DIR / f"hybrid_ssl_beam_akmf_{args.run_name}_summary.csv")
    print("\nSummary:")
