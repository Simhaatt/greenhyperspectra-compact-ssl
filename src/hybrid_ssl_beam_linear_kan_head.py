...
RESULTS_DIR = ROOT / "results" / "hybrid_ssl_beam_linear_kan"
...
class KANLayer(nn.Module):
    """KAN-style radial basis layer used after a stabilizing linear projection."""
...
class HybridLinearKANHead(nn.Module):
    """Linear -> KAN -> Linear head for hybrid BeamSearch + SSL features."""

    def __init__(self, in_features: int, config: LinearKANConfig) -> None:
        super().__init__()
        self.net = nn.Sequential(
            ...
            KANLayer(config.hidden_1, config.kan_hidden, config.grid_size),
...
def fit_predict(
    ...
    model = HybridLinearKANHead(x_train.shape[1], config).to(device)
...
def run_trait(
    ...
    print(f"\nTrait={target} | train={len(y_train)} test={len(y_test)} linear_kan_features={x_train.shape[1]} device={device}")
    for seed in seeds:
        pred, timing = fit_predict(x_train, y_train, x_test, seed, config, device)
        row = {
            "target": target,
            "selector": "BeamSearch",
            "model": "Hybrid_BeamK30_SSL_LinearKAN",
            "seed": seed,
            "budget": budget,
            "beam_bands": budget,
...
def save_figures(summary: pd.DataFrame, run_name: str) -> None:
    ...
    for metric in ["r2", "rmse"]:
        ...
        plt.errorbar(
            ...
            linewidth=2,
            capsize=4,
            label="Linear-KAN head",
        )
        plt.xlabel("Trait")
        plt.ylabel(metric.upper())
        plt.title(f"Hybrid BeamSearch K=30 + SSL Linear-KAN head: {metric.upper()}")
        plt.grid(True, alpha=0.25)
        plt.legend()
        plt.tight_layout()
        plt.savefig(FIG_DIR / f"hybrid_ssl_beam_linear_kan_{run_name}_{metric}.png", dpi=240)
        plt.close()


def main() -> None:
    parser = argparse.ArgumentParser(description="Hybrid BeamSearch + SSL embedding with Linear -> KAN -> Linear head.")
    ...
    results_path = RESULTS_DIR / f"hybrid_ssl_beam_linear_kan_{run_name}_results.csv"
    summary_path = RESULTS_DIR / f"hybrid_ssl_beam_linear_kan_{run_name}_summary.csv"
    summary_copy_path = SUMMARY_DIR / f"hybrid_ssl_beam_linear_kan_{run_name}_summary.csv"
