...
RESULTS_DIR = ROOT / "results" / "hybrid_ssl_beam"
...
class HybridMLP(nn.Module):
    def __init__(self, in_features: int, config: HybridConfig) -> None:
        super().__init__()
...
def ssl_embeddings(
    ...
    x = ((x_full.astype(np.float32) - mean) / std).astype(np.float32)
    latent_channels = int(ckpt.get("latent_channels", 128))
    encoder = SpectralEncoder(latent_channels=latent_channels).to(device)
    ...
    with torch.no_grad():
        for (xb,) in loader:
            z = encoder(xb.to(device).unsqueeze(1))
...
def fit_predict(
    ...
    model = HybridMLP(x_train.shape[1], config).to(device)
...
def run_trait(
    ...
    print(f"\nTrait={target} | train={len(y_train)} test={len(y_test)} hybrid_features={x_train.shape[1]} device={device}")
    for seed in seeds:
        pred, timing = fit_predict(x_train, y_train, x_test, seed, config, device)
        row = {
            "target": target,
            "selector": "BeamSearch",
            "model": "Hybrid_BeamK30_SSL_Embedding_MLP",
            "seed": seed,
            "budget": budget,
            "beam_bands": budget,
...
def save_figures(summary: pd.DataFrame, run_name: str) -> None:
    ...
    for metric in ["r2", "rmse"]:
        ...
        plt.errorbar(
            ...
            linewidth=2,
            capsize=4,
            label="Hybrid Beam + SSL embedding",
        )
        plt.xlabel("Trait")
        plt.ylabel(metric.upper())
        plt.title(f"Hybrid BeamSearch K=30 + SSL embedding MLP: {metric.upper()}")
        plt.grid(True, alpha=0.25)
        plt.legend()
        plt.tight_layout()
        plt.savefig(FIG_DIR / f"hybrid_ssl_beam_{run_name}_{metric}.png", dpi=240)
        plt.close()


def main() -> None:
    parser = argparse.ArgumentParser(description="Hybrid BeamSearch selected wavelengths + SSL encoder embedding MLP.")
    ...
    results_path = RESULTS_DIR / f"hybrid_ssl_beam_{args.run_name}_results.csv"
    summary_path = RESULTS_DIR / f"hybrid_ssl_beam_{args.run_name}_summary.csv"
    results.to_csv(results_path, index=False)
    summary.to_csv(summary_path, index=False)
    summary.to_csv(SUMMARY_DIR / f"hybrid_ssl_beam_{args.run_name}_summary.csv", index=False)
    save_figures(summary, args.run_name)

    print("\nSaved:")
    print(results_path)
    print(summary_path)
    print(SUMMARY_DIR / f"hybrid_ssl_beam_{args.run_name}_summary.csv")
    print("\nSummary:")
